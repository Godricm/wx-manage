<template>
  <div v-if="objData.replyType == 'image'">
  ...
  <div v-else-if="objData.replyType == 'voice'">
    <avue-crud
      ref="crud"
      :page="page"
      :data="tableData"
  ...
  <div v-else-if="objData.replyType == 'video'">
    <avue-crud
      ref="crud"
      :page="page"
      :data="tableData"
  ...
  <div v-else-if="objData.replyType == 'news'">
    <div class="waterfall" v-loading="tableLoading">
      <div
      ...
      >
        <wx-news :objData="item.content.articles"></wx-news>
        <el-row class="ope-row">
          <el-button size="mini" type="success" @click="selectMaterial(item)"
            >选择<i class="el-icon-circle-check el-icon--right"></i
          ></el-button>
...
<script>
import { getDataList } from "@/api/wx/wxassets";
...
export default {
  ...
  methods: {
    ...
    getPage(page, params) {
      ...
      getDataList(this.$http.adornParams({
          page: page.currentPage,
          size: page.pageSize,
          type: this.objData.replyType,
        })).then(({ data }) => {
