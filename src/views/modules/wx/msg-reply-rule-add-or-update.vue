  ...
  >
    ...
    >
      ...
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item label="失效时间" prop="effectTimeEnd">
            <el-time-picker
              v-model="dataForm.effectTimeEnd"
              value-format="HH:mm:ss"
            ></el-time-picker>
          </el-form-item>
        </el-col>
      ...
      <el-form-item label="回复内容" prop="replyContent">
        ...
        <el-button
          type="text"
          v-show="'text' == dataForm.replyType"
          @click="addLink"
          >插入链接</el-button
        >
        <el-button
        ...
        >
          从素材库中选择<span
            v-if="
              'miniprogrampage' == dataForm.replyType ||
              'music' == dataForm.replyType
            "
            >缩略图</span
          >
        </el-button>
...
<script>
import { getObj, save } from "@/api/wx/wxautoreply";
  ...
  methods: {
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.ruleId) {
          getObj(this.dataForm.ruleId, this.$http.adornParams()).then(
            ({ data }) => {
    ...
    dataFormSubmit() {
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          save(this.dataForm.ruleId, this.$http.adornData(this.dataForm)).then(
            ({ data }) => {
              if (data && data.code === 200) {
                this.$message({
                  message: "操作成功",
